    ...
    def getCard(self, user):
        ...
        if user.is_designer == True:
            ...
            if user.get_work_style is not None:
                profile_object["workstyle"] = user.get_work_style
            
            if user.get_work_focus is not None:
                profile_object["workfocus"] = user.get_work_focus

            if user.get_work_interest is not None:
                profile_object["workinterest"] = user.get_work_interest

            if user.get_bio is not None:
                profile_object["bio"] = user.get_bio

            if user.get_experience is not None:
                profile_object["experience"] = user.get_experience

            if user.get_institution is not None:
                profile_object["institution"] = user.get_institution

            if user.get_proficiency is not None:
                profile_object["proficiency"] = user.get_proficiency

            userObject['DesignerProfile'] = profile_object
